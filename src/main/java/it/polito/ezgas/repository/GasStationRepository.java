...
public interface GasStationRepository extends JpaRepository<GasStation, Integer> {
    ...
    @Query(value = "SELECT * FROM GAS_STATION WHERE HAS_DIESEL=TRUE AND CAR_SHARING = ?2", nativeQuery = true)
    List<GasStation> findByHasDieselAndCarSharing(String carSharing);

    @Query(value = "SELECT * FROM GAS_STATION WHERE HAS_GAS = TRUE AND CAR_SHARING=?2", nativeQuery = true)
    List<GasStation> findByGasAndCarSharing(String carSharing);

    @Query(value = "SELECT * FROM GAS_STATION WHERE HAS_METHANE = TRUE AND CAR_SHARING=?2", nativeQuery = true)
    List<GasStation> findByMethaneAndCarSharing(String carSharing);

    @Query(value = "SELECT * FROM GAS_STATION WHERE HAS_SUPER = TRUE AND CAR_SHARING=?2", nativeQuery = true)
    List<GasStation> findBySuperAndCarSharing(String carSharing);

    @Query(value = "SELECT * FROM GAS_STATION WHERE HAS_SUPER_PLUS = TRUE AND CAR_SHARING=?2", nativeQuery = true)
